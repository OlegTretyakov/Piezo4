unit mas6279d8.DboMain;

interface
  uses ChipAbstract, MAS6279D8IMS, Stp8PositionDBExtention, mas6279d8.DboBase,
  MeasDataList, MAS6279D8.Matrix, VvarMatrix,
  ...
  TFCPoint= class(TMeasDataItem)
   ...
   public
    ...
    function ErrorsExists(AErrors : TVFErrors):Boolean;stdcall;
  end;
  ...
  TFCitem = class(TMeasDataItem)
   ...
   private
    ...
    function GetFcErrors : TVFErrors;
    procedure SetFcErrors(Value : TVFErrors);
   ...
   public
    ...
    property ItemErrors : TVFErrors read GetFcErrors write SetFcErrors;
    property Vdd: double  read GetVdd write SetVdd;
    property VC: double  read GetVC write SetVC;
  end;
  ...
  TVvarFCObj = class(TMeasDataItem)
   private
    function GetItemError : TVFErrors;
    procedure ErrorInclude(AError: TVFError);
   public
    property ItemErrors : TVFErrors read GetItemError;
  end;
  ...
  TMasIterationResults  = class;
  TCalcCurves = class(TSolution)
   private
    fIterationMas: TMasIterationResults;
    fRootObj : TMeasureSolutionCurves<TCalcCurves>;
    fVvar,
    fFreq : TMeasDataList;
    fdReg, fPrior : Double;
   public
    constructor Create(const AIteration : TMasIterationBase; const ARootIdx : Byte);override;
    destructor Destroy; override;
    property IterationMas : TMasIterationResults read fIterationMas;
    property RootObj : TMeasureSolutionCurves<TCalcCurves> read fRootObj;
    procedure CalcCurves;overload;
    procedure CalcCurves(const ARegisters :TMas6279D8Registers);overload;
  ...
  TMeasureSolutionCurves<T : TCalcCurves> = class(TMeasure<T>)
   ...
   public
    ...
    property CharactTable : TCTMasD8 read fCharactTable;
  end;


  TMasIterationResults = class(TMasIteration<TCalcCurves, TMeasureSolutionCurves<TCalcCurves>>)
   private
  ...
  TMasResultsDBObj = class(TmasDbExtention)
   private
    fIterations : TObjectList<TMasIterationResults>;
   protected
   ...
   public
    ...
    property Iterations : TObjectList<TMasIterationResults> read fIterations;
  end;
...
function TFCitem.GetFcErrors: TVFErrors;
var
...
procedure TFCitem.SetFcErrors(Value: TVFErrors);
var
vTmp : TVFErrorsAdapter;
begin
  vTmp.i := 0;
...
function TVvarFCObj.GetItemError: TVFErrors;
var
...
begin
  ...
  fFreqCurve := TMeasDataList.Create;
...
procedure VFCFill(const ADB: TEmbSQLDataBase; const ADBObj : TPositionDBExtention;
                        const Iter : TMasIterationResults; const Dest: TFCMeasResults);
const vSqlText =
      'SELECT  IZM_NUM, POINTIDX, TEMPR, '+
      'VDD, VC, VOLTAGE, FREQ, FREQDISP, ERR_CODE FROM VVAR_FC '+
      'WHERE ITER_NUM = :ITER_NUM ORDER BY IZM_NUM';
...
vVDD_VCErrors : word;
  ...
  begin
    ...
    begin
      ...
      begin
        vFCVarY := Iter.Options.ErrorLimits.VvarMin - C_Vvar_Facking_Error_Value;
        vFCNominalOut := True;
      end else
      if (ADBObj.Nominal > vVvarCurve.Splines[0].LastX) then
      begin
        vFCVarY := Iter.Options.ErrorLimits.VvarMax + C_Vvar_Facking_Error_Value;
        vFCNominalOut := True;
      end else
...
procedure TMasResultsDBObj.AfterCreate;
begin
  inherited;
  fIterations := TObjectList<TMasIterationResults>.Create;
end;

procedure TMasResultsDBObj.BeforeDestroy;
begin
  fIterations.Clear;
...
class function TMasResultsDBObj.DefaultIterationClass: TIterationClass;
begin
  Result := TMasIterationResults;
end;

procedure TMasResultsDBObj.Load(const AFileName : string; ACalcResults : Boolean);
var
vDB: TEmbSQLDataBase;
vPosData : TMemIniFile;
vFormatSettings: TFormatSettings;
vIterResults : TMasIterationResults;
i : Integer;
begin
  fIterations.Clear;
  ...
  try
    ...
    try
      ...
      and IsEqualGUID(ChipGUID, MAS6279D8.Consts.C_GUID)
      and (ItersCount > 0) then
      begin
        i := 0;
        while i < ItersCount do
        begin
          vIterResults := TMasIterationResults.Create(self);
...
function CalcDeltaReg(const AInitVect, ADestVect :TMas6279D8Registers; const RegWeighs : TRegistersDouble):double; overload;
var
i : byte;
...
procedure TCalcCurves.CalcCurves(const ARegisters: TMas6279D8Registers);
var
vPointIdx :Word;
vCurveIdx : integer;
vTemprI,
...
begin
  ...
  if (Self.Vvar.Count < 1)
    or(IterationMas.FCResults.Curves.Count < 1)
  ...
  try
    fFreq.ClearData;
    if IterationMas.MeasPoints.Count < 1 then
      Exit;
    for vPointIdx := 0 to IterationMas.MeasPoints.Count -1 do
    begin
      vTemprI := IterationMas.MeasPoints[vPointIdx].PointValue;
      if not IterationMas.FCResults.Curves.XExists(vTemprI, 0.5, vCurveIdx) then
        Continue;
      vFC := IterationMas.FCResults.Curves[vCurveIdx] as TFCPoint;
      if (vFC.FreqCurve.Splines.Count < 1)
      or (vFC.FreqCurve.Splines[0].Count < 3) then
       Continue;
      vVarCalcX := Self.Vvar.YX(vTemprI);
      if (vVarCalcX >= IterationMas.Options.ErrorLimits.VvarMin)
      and (vVarCalcX <= IterationMas.Options.ErrorLimits.VvarMax) then
      begin
        vFreqCalcX := vFC.FreqCurve.YX(vVarCalcX);
        vCurr_ppm_Error := ppm(vFreqCalcX, IterationMas.DBExt.Nominal);
        vCO := fFreq.NewItem as TCalcErrorObj;
        vCO.XValue := vTemprI;
        vCO.Freq := vFreqCalcX;
        vCO.YValue := vCurr_ppm_Error;
        vVarForceCalcX := IterationMas.FCResults.Force.YX(vTemprI);
    ...
    begin
      ...
      vFreq25 := 1;
    fdReg := CalcDeltaReg(fRootObj.Registers, ARegisters, IterationMas.Options.CalcOptions.RegistersWeighs);
  end;
  ...
  case IterationMas.Options.CalcOptions.CalcMethod  of
    cmGeneral:
    begin
      while vCurveIdx < Freq.Count do
      begin
        if Freq.QueryItem(vCurveIdx, TCalcErrorObj, vCO) then
        begin
          vTemprI := vCO.XValue;
          if IterationMas.MeasPoints.Find(vTemprI, 0.7, vPointIdx)
          and  IterationMas.MeasPoints[vPointIdx].WorkPoint then
          begin
    ...
    begin
      ...
      fPrior := Results.dF + fdReg + IterationMas.Options.CalcOptions.cm25Coeff * Absppm(vFreq25, IterationMas.DBExt.Nominal);
    end;
  end;
end;
...
constructor TCalcCurves.Create(const AIteration: TMasIterationBase; const ARootIdx : Byte);
begin
  inherited Create(AIteration, ARootIdx);
  fVvar := TMeasDataList.Create;
  fFreq := TMeasDataList.Create;
  fIterationMas := TMasIterationResults(AIteration);
  fRootObj := fIterationMas.MeasSolutions[RootIdx];
end;
...
constructor TMeasureSolutionCurves<T>.Create(const AIteration: TMasIterationBase);
begin
...
{ TMasIterationResults }

constructor TMasIterationResults.Create(const ADBExt: TPositionDBExtention);
begin
...
destructor TMasIterationResults.Destroy;
begin
  fFCResults.Clear;
  fTC_OFF.ClearData;
...
procedure TMasIterationResults.Load(const ADB: TEmbSQLDataBase; ACalcResults : Boolean);
var
i, j : Word;
...
procedure TMasIterationResults.RecalcMeasureResults;
var
i, j : Word;
vFOCurr : TFinalTestObj;
vTmp, vF, vFreq25 : Double;
vCurrIterIdx : Integer;
vPrevIter : TMasIterationResults;
begin
  Results.dF := 1E6;
  Results.dF25 := 1E6;
  Results.Iter_dF25 := 0;
  ...
  begin
    vPrevIter := nil;
    if (DBExt is TMasResultsDBObj)
    and (TMasResultsDBObj(DBExt).Iterations.Count > 1) then
    begin
      vCurrIterIdx := TMasResultsDBObj(DBExt).Iterations.IndexOf(self);
      if (vCurrIterIdx <> -1)
      and (vCurrIterIdx > 0) then
        vPrevIter := TMasResultsDBObj(DBExt).Iterations[vCurrIterIdx-1];
    end;
...
procedure TMasIterationResults.CalcIterStab(ACurvePrev, ACurveCurr : TMeasDataList);
var
...
function TMasIterationResults.TotalErrorPercens: double;
var
vMeasIdx,
vTotalMeas,
vTotalError : word;
