...
interface
   uses
    ...
    ChamberInterfaces, VdpDMAccess, MAS6279D8IMS, MAS6279D8.Consts,
    Stp8PositionDBExtention, mas6279d8.DboBase, mas6279dStp8dboExtention, AbstractBoardInterface,
  ...
  Tmasd8AbstractTestThread = class;
  TProcessedPosition = class(TObject)
   public
    TestThread : Tmasd8AbstractTestThread;
    Succ : Boolean;
    Position : IPosition;
    ...
    Chip : TMas6279D8Registers;
    CardProcessOptions : TStp8masProcessDBOptions;
    DbExt : TStp8masDbExtention;
    constructor Create(const ATestThread : Tmasd8AbstractTestThread); virtual;
  ...
  Tmasd8AbstractTestThread = class(TStpThread)
  private
   ...
   protected
    ...
    property ProcessOptions : TObjectList<TStp8masProcessDBOptions> read fProcessOptions;
   public
...
procedure Tmasd8AbstractTestThread.FreqMeterEvent(Sender: TObject; Event: TGUID;
  Params: Pointer);
begin
...
function Tmasd8AbstractTestThread.FreqStartAndResume: Boolean;
var
vTimeOut : Cardinal;
vStarted : Boolean;
vAttemptCount : Byte;
...
procedure Tmasd8AbstractTestThread.PosControllerEvent(Sender: TObject; Event: TGUID;
  Params: Pointer);
begin
...
function Tmasd8AbstractTestThread.PositionClass: TProcessedPositionClass;
begin
  Result := TProcessedPosition;
end;

procedure Tmasd8AbstractTestThread.Stop;
begin
  inherited Stop;
...
procedure Tmasd8AbstractTestThread.OnProgrammerEvent(Sender: TObject; Event: TGUID;
  Params: Pointer);
var
vLogger : IdmLogger;
begin
...
function Tmasd8AbstractTestThread.GetActiveCount:Word;
var
vIdx : Word;
begin
  Result := 0;
...
function Tmasd8AbstractTestThread.GetSuccCount:Word;
var
vIdx : Word;
begin
  Result := 0;
...
function Tmasd8AbstractTestThread.RemoveInactive:Boolean;
var
vIdx : integer;
begin
  result := False;
...
procedure Tmasd8AbstractTestThread.DropUnSuccPositions;
begin
  if Terminated then
    exit;
...
procedure Tmasd8AbstractTestThread.DropInProtectPositions;
begin
  if Terminated then
    exit;
  if fProcessPositions.Count < 1 then
...
function Tmasd8AbstractTestThread.SendPositionsVoltagesAndResume(
...
function Tmasd8AbstractTestThread.SleepAndResume(ASleepTime : Cardinal):Boolean;
begin
  if Terminated then
  begin
    result := false;
...
function Tmasd8AbstractTestThread.SendDisablesActive(ATimeOut: Cardinal): boolean;
var
vApIdx,
vAffectedCount : Word;
vPosIdx : integer;
...
function Tmasd8AbstractTestThread.SendEnables(ATimeOut: Cardinal) : boolean;
var
vApIdx,
vAffectedCount : Word;
vPosIdx : integer;
...
function Tmasd8AbstractTestThread.CheckPowerVoltages(var SuccCount : Word):boolean;
var
...
begin
  ...
  if not Result then
    fBoardLogger.Log(lWarning, Format('check voltage error. succ count:%d, total count %d',[SuccCount, fProcessPositions.Count]))
  else
...
procedure Tmasd8AbstractTestThread.FillProcessPositions;
function GetProcessOptions(AID : Integer; var oOptions : TStp8masProcessDBOptions):boolean;
var
vPoIdx : Word;
begin
  vPoIdx := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      or (not vProcessPosition.Extentions.Find(TMas6279D8Registers, vProcessPosition.Chip))
      ...
      or (not vProcessPosition.Extentions.Find(TStp8masDbExtention, vProcessPosition.DbExt))
...
procedure Tmasd8AbstractTestThread.UpdatePositionsState;
begin
  fPositionsList.BeginUpdate;
  fPositionsList.EndUpdate;
end;

procedure Tmasd8AbstractTestThread.LogPositonsFreqs;
var
vIdx, vErrors : word;
vLogStr : string;
vProcessPosition : TProcessedPosition;
...
procedure Tmasd8AbstractTestThread.LogPositonsVoltagesStates;
var
vIdx : word;
vLogging : Boolean;
vLogStr : string;
...
procedure Tmasd8AbstractTestThread.LogPositonsRegisters;
var
vIdx : word;
vBitIdx : Byte;
vLogging : Boolean;
...
function Tmasd8AbstractTestThread.ProgrammerReadRomAndResume(
  ATimeOut: Cardinal): Boolean;
var
...
function Tmasd8AbstractTestThread.ProgrammerReadWorkAndResume(
  ATimeOut: Cardinal): Boolean;
var
  vProgrammerError : TProgrammerError;
  vAttemptCount : Byte;
...
function Tmasd8AbstractTestThread.ProgrammerWriteRomAndResume(
  ATimeOut: Cardinal): Boolean;
var
  vProgrammerError : TProgrammerError;
  vAttemptCount : Byte;
...
function Tmasd8AbstractTestThread.ProgrammerWriteWorkAndResume(
  ATimeOut: Cardinal): Boolean;
var
  vProgrammerError : TProgrammerError;
  vAttemptCount : Byte;
...
function Tmasd8AbstractTestThread.ProgrammerZState(ATimeOut: Cardinal): Boolean;
var
  vProgrammerError : TProgrammerError;
  vAttemptCount : Byte;
begin
...
function Tmasd8AbstractTestThread.IndexOfProcessedPosition(ANum : Byte):integer;
begin
  result := 0;
...
procedure Tmasd8AbstractTestThread.ReInitPositions;
var
...
procedure Tmasd8AbstractTestThread.SetActiveProcessedPositions;
var
...
procedure Tmasd8AbstractTestThread.Execute;
var
vEB : IEventBus;
vLinkedEvents : TLinkedEvents;
begin
  {$IFDEF DEBUG}
    TThread.NameThreadForDebugging('MAS 6279D8 measure thread');
  {$ENDIF}
  ...
  or (not fPositionsProgrammer.ProgrammerExists(MAS6279D8.Consts.C_GUID))) then
  begin
    fDMAccess := nil;
    fChamberProcess := nil;
  ...
  try
    ...
    try
      ...
      if not IsEqualGUID(fPositionsProgrammer.ActiveProgrammer, MAS6279D8.Consts.C_GUID) then
      begin
        fPositionsProgrammer.ActiveProgrammer := MAS6279D8.Consts.C_GUID;
        if not SleepAndResume(1500) then
          Exit;
        if not IsEqualGUID(fPositionsProgrammer.ActiveProgrammer, MAS6279D8.Consts.C_GUID) then
        begin
          fBoardLogger.Log(lWarning, Format('"%s" can not started. '+
                              'Programmer for MAS6279D8 not supported or not switched properly',
...
constructor TProcessedPosition.Create(const ATestThread : Tmasd8AbstractTestThread);
begin
  inherited Create;
...
begin
  if ((not TMas6279D8Registers.ReadConstant('MeasProcessorSettings','DelayBeforeVoltageMeasure',vVoltage))
  or (vVoltage < 1)) then
    Voltage := 5000
  else
    Voltage := vVoltage * 1000;
  if ((not TMas6279D8Registers.ReadConstant('MeasProcessorSettings','DelayBeforeFreqMeasure',vFreq))
  or (vFreq < 1)) then
    Freq := 5000
  else
    Freq := vFreq * 1000;
  if ((not TMas6279D8Registers.ReadConstant('MeasProcessorSettings','DelayAfterEnable',vAfterEnable))
