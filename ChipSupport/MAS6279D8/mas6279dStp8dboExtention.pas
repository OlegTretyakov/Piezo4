unit mas6279dStp8dboExtention;

interface
  uses
  System.Classes, System.UITypes, System.Generics.Collections,
  System.SysUtils, System.IniFiles,
  Stp8PositionDBExtention, MAS6279D8IMS, mas6279d8.DboBase,
  Stp8ProcessDBOptionsInterface, VdpDMAccess;
   type
   TStp8masProcessDBOptions = class(TmasProcessDBOptions,
                             IInterface,
   ...
   public
    ...
    {IInterface}
     ...
     function Stp8_Load(const ADMAccess : IDMAccess;
                        const AConnection : TvdConnection;
                        const ACardID : Integer;
                        const ATextOptions : TStrings = nil):boolean; stdcall;
     function Stp8_Save(const ADMAccess : IDMAccess;
                        const AConnection : TvdConnection;
                        const ACardID : Integer): boolean; stdcall;
   end;
   ...
   TMasStepsResult = (msrWRTestOk, msrWRTestFail,
                    msrFreqTestOk, msrFreqTestFail,
                    msrInfTrimOk, msrInfTrimFail,
                    msrCDACTrimOk, msrCDACTrimFail,
                    mrsFinalTestOk, mrsFinalTestFail);
   TMasStepsResults = set of TMasStepsResult;

   TStp8masDbExtention = class(TmasDbExtention)
   private
     fLastIter : TMasIteration<TSolution, TMeasure<TSolution>>;
     procedure SetNominal(const Value: double);
     function GetStepsResults: TMasStepsResults;
     procedure SetStepsResults(const Value: TMasStepsResults);
    ...
    public
     ...
     property LastIter : TMasIteration<TSolution, TMeasure<TSolution>> read fLastIter;
     property Nominal : double read fNominal write SetNominal;
     property StepsResults : TMasStepsResults read GetStepsResults write SetStepsResults;
     procedure StepResultInclude(Value : TMasStepsResult);
     procedure StepResultExclude(Value : TMasStepsResult);
   ...
   TMasPositionDBView = class(TPositionDBView)
    private
...
C_PreparePointOptionsMethodsTxt : array [TPreparePointProcOption] of string = ('Inf Trim',
                                                'CDAC Trim',
                                                'Inf Trim');
C_InRangeProcOptionTxt : array [TInRangeProcOption] of string = (
                                                              'Vvar Charact',
                                                              'Vvar FC',
                                                              'Vvar Curr',
                                                              'Freq Curr',
...
TInstallResource = record
  ...
  InsSP : TvdStoredProc;
  CardOptions : TmasProcessDBOptions;
  IniSect : TStringList;
...
begin
  ...
  try
    ...
    or (not (vResource.Card.ProcOptions is TmasProcessDBOptions))) then
    begin
    ...
    vResource.CardOptions := TmasProcessDBOptions(ACard.ProcOptions);
...
begin
  ...
  try
    ...
    begin
      ...
      Result.DbView := TMasPositionDBView.Create;
      vDbExt := TStp8masDbExtention(Result.DbView.DbExtention);
      ...
      vDbExt.fChipGUID := MAS6279D8.Consts.C_GUID;
      ...
      try
        ...
        vNewPosData.ChipGUID := MAS6279D8.Consts.C_GUID;
        vNewPosData.SerialNum := ASerial;
        vDbExt.SetViewerPlugin(vResource.StartOptionsIni, 'Mas6279D8Stp8rv.bpl');
...
begin
  Result := TStp8masProcessDBOptions;
...
begin
  ...
  try
    ...
    try
      ...
      if result then
      begin
        vDbView := TMasPositionDBView.Create;
        vDbExt := vDbView.DbExtention as TStp8masDbExtention;
        ...
        try
          ...
          and IsEqualGUID(vDbView.DbExtention.ChipGUID, MAS6279D8.Consts.C_GUID);
          vDbExt.fSerialNum := vResource.SelectQR.FieldByName('rnum').AsString;
          vDbExt.fBoard_SN := vResource.SelectQR.FieldByName('board').AsInteger;
          vDbExt.fBoard_Pos := Byte(vResource.SelectQR.FieldByName('boardpos').AsInteger);
          vDbExt.fBoardState := Byte(vResource.SelectQR.FieldByName('status').AsInteger);
...
vChipAbstract : TMas6279D8Registers;
vDBExt : TStp8masDbExtention;
vDB : TEmbSQLDataBase;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if vPosExtentions.Find(TMas6279D8Registers, vChipAbstract)
        {and (IsEqualGUID(vChipAbstract.ChipGUID, MAS6279D8.Consts.C_GUID))} then
        begin
          vPosExtentions.Install(TStp8masDbExtention, vDBExt);
                ...
                begin
                  ...
                  try
                    ...
                    try
                      ...
                      or (not IsEqualGUID(vDBExt.ChipGUID, MAS6279D8.Consts.C_GUID)) then
                        vNeedDelete := True
                      else
                      begin
                    ...
                    vChipAbstract.Registers.DA := 1;
                    vChipAbstract.Registers.CLK := 1;
...
begin
  ...
  ReadControlledBitsIndex(TMas6279D8Registers, 'Stp8AddMeasRegisters', False, vBitsIdx);
  try
    try
      vQR.Prepare;
      ...
      begin
        ...
        begin
          ...
          begin
            if vFile.ValueExists(vST[vSectIdx], TMas6279D8Registers.BitName(vBitsIdx[vArrIdx])) then
            begin
              vAddValue := vFile.ReadInteger(vST[vSectIdx], TMas6279D8Registers.BitName(vBitsIdx[vArrIdx]), 0);
              if (vAddValue <> 0) then
              begin
...
vIterSaveFields : mas6279d8.DboBase.TFieldOptions;
vPosLog : TPosLogItem;
vCardOptions : TStp8masProcessDBOptions;
vNewMeasSol : TMeasure<TSolution>;
vCardOptionsList : TObjectList<TStp8masProcessDBOptions>;
vAddMeasPattern : TAddMeasPattern;
vAddMeasPatterns : TObjectList<TAddMeasPattern>;
function GetCardOptions(ACardId : Integer; var oOptions : TStp8masProcessDBOptions) : boolean;
var
vIdx : Word;
begin
  Result := false;
  ...
  if not result then
  begin
    oOptions := TStp8masProcessDBOptions.Create;
...
begin
  ...
  try
    case AState of
      ...
      begin
        ...
        try
          ...
          begin
            ...
            and vPosExtentions.Find(TStp8masDbExtention, vDBExt)
      ...
      begin
        vPosLog.Result := 'OK';
        vCardOptionsList := TObjectList<TStp8masProcessDBOptions>.Create;
        vAddMeasPatterns := TObjectList<TAddMeasPattern>.Create;
        try
          for i := 0 to APositionList.Count -1 do
          begin
            if APositionList.QueryItem(i, IExtentions, vPosExtentions)
            and vPosExtentions.Find(TStp8masDbExtention, vDBExt) then
            begin
...
function TStp8masProcessDBOptions.QueryInterface(const IID: TGUID;
  out Obj): HResult;
begin
...
function TStp8masProcessDBOptions.Stp8_Load(const ADMAccess: IDMAccess; const AConnection : TvdConnection;
    ...
    try
      ...
      begin
        try
          ...
          vIpOp.MeasInf := vIniFile.ReadBool(MAS6279D8.Consts.C_GUID.ToString, 'MeasInf', false);
          vIpOp.WorkPoint := vIniFile.ReadBool(MAS6279D8.Consts.C_GUID.ToString, 'WorkPoint', false);
          vSelPattern.ParamByName('POINT_ID').AsInteger := vIpOp.PointID;
          vSelPattern.ParamByName('PATTERN_TYPE').AsSmallInt := 4;
          vSelPattern.ParamByName('CHIP_GUID').AsString := MAS6279D8.Consts.C_GUID.ToString;
          try
            vSelPattern.Open;
...
function TStp8masProcessDBOptions.Stp8_Save(const ADMAccess: IDMAccess; const AConnection : TvdConnection;
  ...
  try
    ...
    begin
      ...
      try
        ...
        begin
          ...
          vSelectQR.Close;
          vIniFile.SetStrings(vSt);
          vIniFile.WriteBool(MAS6279D8.Consts.C_GUID.ToString, 'MeasInf', vIpOp.MeasInf);
          vIniFile.WriteBool(MAS6279D8.Consts.C_GUID.ToString, 'WorkPoint', vIpOp.WorkPoint);
          vSt.Clear;
    ...
    begin
      ...
      begin
        ...
        vSelectQR.ParamByName('CHIP_GUID').AsString := MAS6279D8.Consts.C_GUID.ToString;
        vSelectQR.Open;
        if vSelectQR.RecordCount > 0 then
        begin
          vOldpattern := vSelectQR.FieldByName('PATTERN_ID').AsInteger;
...
function TStp8masProcessDBOptions._AddRef: Integer;
begin
  result := -1;
end;

function TStp8masProcessDBOptions._Release: Integer;
begin
  result := -1;
end;

{ TMasPositionDBViewClass }


class function TMasPositionDBView.PositionDBExtentionClass: TPositionDBExtentionClass;
begin
  result := TStp8masDbExtention;
end;
...
function TMasPositionDBView.InProgMode: Boolean;
var
vDbExt : TStp8masDbExtention;
begin
  vDbExt := DbExtention as TStp8masDbExtention;
  Result := vDbExt.InProgQueueState.pqState;
end;

function TMasPositionDBView.LastIterSucc: Boolean;
var
vDbExt : TStp8masDbExtention;
begin
  vDbExt := DbExtention as TStp8masDbExtention;
...
class procedure TMasPositionDBView.ListLegendFiller(
...
procedure TMasPositionDBView.UpdateLegendValues(SelectedCardId: Integer;
...
procedure TStp8masDbExtention.AfterCreate;
begin
  inherited;
  InProgQueueState.pqState := False;
  fLastIter := TMasIteration<TSolution, TMeasure<TSolution>>.Create(self);
end;

procedure TStp8masDbExtention.BeforeDestroy;
begin
  inherited;
  FreeAndNil(fLastIter);
end;

function TStp8masDbExtention.ReadPosData(const ADB: TEmbSQLDataBase): Boolean;
begin
...
function TStp8masDbExtention.RecostructionRequest: boolean;
begin
  result := (ItersCount > 0)
...
TMasStepsResultsAdapter = record
  case integer of
  0: (s: TMasStepsResults);
  1: (i: Cardinal);
end;

function TStp8masDbExtention.GetStepsResults: TMasStepsResults;
var
...
procedure TStp8masDbExtention.SetStepsResults(const Value: TMasStepsResults);
var
vAd : TMasStepsResultsAdapter;
begin
  vAd.i := 0;
...
procedure TStp8masDbExtention.StepResultExclude(Value: TMasStepsResult);
var
vTmp : TMasStepsResults;
begin
  vTmp := StepsResults;
  Exclude(vTmp, Value);
  StepsResults := vTmp;
...
procedure TStp8masDbExtention.StepResultInclude(Value: TMasStepsResult);
var
vTmp : TMasStepsResults;
begin
  vTmp := StepsResults;
...
function TStp8masDbExtention.DeletePosition(const AConnection : TvdConnection;
  const ADMAccess: IDMAccess): boolean;
var
vDelQR : TvdQuery;
begin
  result := false;
...
function TStp8masDbExtention.NewIterRequest: boolean;
begin
...
procedure TStp8masDbExtention.SetFatalError(Value: boolean);
begin
  fFatalError := Value;
end;

procedure TStp8masDbExtention.SetNominal(const Value: double);
begin
  fNominal := Value;
end;
