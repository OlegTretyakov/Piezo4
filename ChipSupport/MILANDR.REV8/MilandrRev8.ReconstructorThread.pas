...
type
TMilRev8Reconstructor = class(TReconstructorAbstractThread)
 private
  ...
  function GetProcessOptions(ACardID : Integer; var oOptions : TStp8milProcessDBOptions): boolean;
...
TRecostructionObjects = packed record
  Reconstructor : TMilRev8Reconstructor;
  MeasObj : TStp8milDbExtention;
  DbObj : TMilResultsDBObj;
  Iter : TMilIterationResults;
  RangeOptions : TInRangeProcOptionSet;
  CurrMeasure : TRootMeasureObj;
  DestRegisters : TMilandrRev8Registers;
  ToleranceGroup : TToleranceGroupEx;
  ProcessOptions : TStp8milProcessDBOptions;
  AddProgQR : TvdQuery;
end;
...
{ TMilRev8Reconstructor }

procedure TMilRev8Reconstructor.AfterCreate;
begin
  fIniParams.LoadIni;
end;

function TMilRev8Reconstructor.GetTolerance(ACardID : Integer): TToleranceGroupEx;
var
vPoIdx : Word;
vPoFindSucc : Boolean;
begin
...
function TMilRev8Reconstructor.GetProcessOptions(ACardID : Integer; var oOptions: TStp8milProcessDBOptions): boolean;
var
vPoIdx : Word;
vOptions : TStp8milProcessDBOptions;
begin
  vPoIdx := 0;
  Result := false;
  ...
  try
    vOptions := TStp8milProcessDBOptions.Create;
...
function TMilRev8Reconstructor.GetRegistersWeightsGroup(AID : Integer):TRegistersWeightsGroup;
var
vPoIdx : Word;
vPoFindSucc : Boolean;
begin
...
function Minimize(AMinimizeVector, ASSVect : pGSL_vector;
                          ...
                          MinimizeThread : TMilRev8Reconstructor) : boolean;
var
    ...
    try
      ...
      begin
        ...
        MinimizeThread.CalcLogger.Log(lTrace,
          ...
          [MinimizeThread.fProcessorBitIdx, sLog, vFminimizer_size, GslStatusName(vSizeStatus),
          vError,
          vParams.CurrMeasure.CalcSolutions.Last.dReg,
          vParams.CurrMeasure.CalcSolutions.Last.Results.dF,
          vParams.CurrMeasure.CalcSolutions.Last.Results.dF25]));
...
procedure TMilRev8Reconstructor.Execute;
var
vParams : pRecostructionObjects;
vCanWhile : Boolean;
...
begin
  {$IFDEF DEBUG}
    TThread.NameThreadForDebugging('Milandr rev.8 reconstructor thread');
  {$ENDIF}
  Priority := tpHigher;
  ...
  TMilandrRev8Registers.ReadControlledBitsIndex('Stp8ChractRegisters', False, fMinimizationRegisters);
  ...
  vParams.DbObj := TMilResultsDBObj.Create(nil);
  ...
  vParams.DestRegisters := TMilandrRev8Registers.Create(nil);
    ...
    try
      ...
      try
        ...
        begin
          ...
          if (not (vItem.MeasuredObj is TStp8milDbExtention)) then
          begin
            vItem.State := isSuspended;
            Continue;
          end;

          vParams.MeasObj := vItem.MeasuredObj as TStp8milDbExtention;
          try
          ...
          if not IsEqualGUID(vParams.DbObj.ChipGUID, MilandrRev8.Consts.C_GUID) then
          begin
            vItem.State := isSuspended;
            if Assigned(CalcLogger) then
              CalcLogger.Log(lWarning,
                              Format('Processor idx:%d. Board:%d, Pos:%d. not is Milandr rev.8 - Suspended',
          ...
          if (rpVvarFC in vParams.RangeOptions) then
          begin
            vTotalMeas := vParams.Iter.FCResults.Force.TotalCount;
            vWorksCount := vParams.Iter.MeasPoints.Count;
            vErrorCount := vParams.Iter.FCResults.Force.ErrorsCount;
            if (vTotalMeas < vWorksCount)  then
            begin
              Inc(vErrorCount, vWorksCount - vTotalMeas);
              vTotalMeas := vWorksCount;
            ...
            if (vTotalMeas > 0) and (vErrorCount > 0) then
            begin
              if not vParams.Iter.Errors.Find(medVvarForce, vErrorItem) then
                vErrorItem := vParams.Iter.Errors.NewItem(medVvarForce);
              vErrorItem^.TotalCount := vTotalMeas;
              vErrorItem^.ErrCount := vErrorCount;
            end;
          end;
                      ...
                      begin
                        ...
                        begin
                          ...
                          if vBitIdx = Low(fMinimizationRegisters) then
                            sLog := sLog + Format('%s = %d',
                            [C_RegNames[fMinimizationRegisters[vBitIdx]] ,
                            FixedTrunc(gsl_vector_get (vMinorBest.Vector, vVecIdx))])
                          else
                            sLog := sLog + Format(', %s = %d',
                            [C_RegNames[fMinimizationRegisters[vBitIdx]] ,
                            FixedTrunc(gsl_vector_get (vMinorBest.Vector, vVecIdx))]);
