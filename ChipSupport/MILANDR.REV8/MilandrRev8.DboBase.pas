...
uses
  ...
  MilandrRev8IMS, System.Generics.Defaults;
 type

    TStartTestProcOption = (cpoWRTest, cpoFreqTest);
    TPreparePointProcOption = (ncCDAC);
    TInRangeProcOption = (rpVvarCurrMeas, rpVvarCharactMeas, rpVvarFC,
    ...
    TmilProcessDBOptions = class(TStp8ProcessDBOptions)
     protected
     ...
     public
      ControlOptions : record
        ...
        AdditionalMeasurePatternID,
        VVFPatternID : Integer;
    ...
    public
      ...
      constructor Create;
      destructor Destroy; override;
      procedure ReadFromCardOptions(const ASource : TmilProcessDBOptions);
    ...
    TMilIterationBase = class(TIteration)
     public
    ...
    TMilIterationRegisters= class(TObject)
     private
      fRegisters : TMilandrRev8Registers;
      fIteration : TMilIterationBase;
     protected
     ...
     public
      ...
      constructor Create(const AIteration : TMilIterationBase); reintroduce; virtual;
      destructor Destroy;override;
      ...
      property Registers : TMilandrRev8Registers read fRegisters;
      property Iteration : TMilIterationBase read fIteration;
    end;

    TMeasures<T : TMilIterationRegisters> = class(TObjectList<T>)
     private
      fIteration : TMilIterationBase;
      function GetItem(AIndex : Word):T;
     public
      constructor Create(const AIteration : TMilIterationBase); reintroduce; overload; virtual;
      constructor Create(const AIteration : TMilIterationBase; const AComparer: IComparer<T>; AOwnsObjects: Boolean = True); reintroduce; overload; virtual;
      constructor Create(const AIteration : TMilIterationBase; const Collection: TEnumerable<T>; AOwnsObjects: Boolean = True); reintroduce; overload; virtual;
    ...
    TSolution = class(TMilIterationRegisters)
     private
      fRootIdx : Byte;
     public
      {RootIdx : byte;
      ...
      constructor Create(const AIteration : TMilIterationBase; const ARootIdx : Byte); reintroduce; virtual;
    ...
    TMeasure<TCalc : TSolution> = class(TMilIterationRegisters)
     strict private
    ...
    TIterPointsList = class(TObjectList<TMilMeasPoint>)
     private
      fIteration : TMilIterationBase;
     public
      constructor Create(const AOwner: TMilIterationBase); reintroduce;
    ...
    TMilIteration<TCalc : TSolution;
                          TMeas : TMeasure<TCalc>>  = class(TMilIterationBase)
     private
    ...
    TmilDbExtention = class(TPositionDBExtention)
     private
      fStartOptions : TIterOptions;
      fInitRegisters : TMilandrRev8Registers;
     protected
      fFatalError : boolean;
     ...
     public
      ...
      property InitRegisters : TMilandrRev8Registers read fInitRegisters;
...
constructor TmilProcessDBOptions.Create;
begin
  inherited;
  ControlOptions.InPointOptions := TList<TInPointOption>.Create;
end;

destructor TmilProcessDBOptions.Destroy;
begin
  FreeAndNil(ControlOptions.InPointOptions);
  inherited;
end;

procedure TmilProcessDBOptions.Read(const ASource: TMemIniFile;
...
procedure TmilProcessDBOptions.Assign(const ASource: TStp8ProcessDBOptions);
var
vSource : TmilProcessDBOptions;
vIdx : Byte;
vOp : TInPointOption;
vMm : TMeasureMode;
begin
  inherited;
  if not (ASource is TmilProcessDBOptions) then
    Exit;
  vSource := ASource as TmilProcessDBOptions;
  ...
  ControlOptions.VVFPatternID := vSource.ControlOptions.VVFPatternID;
...
function TmilProcessDBOptions.Equals(const ASource: TStp8ProcessDBOptions): Boolean;
var
vSource : TmilProcessDBOptions;
i : Byte;
...
begin
  ...
  if Result then
    Result := (ASource is TmilProcessDBOptions);
  if not Result then
    Exit;
  vSource := ASource as TmilProcessDBOptions;
  Result := (ControlOptions.AllowChangeCDACNominal = vSource.ControlOptions.AllowChangeCDACNominal)
    ...
    and (ControlOptions.VVFPatternID = vSource.ControlOptions.VVFPatternID)
...
procedure TmilProcessDBOptions.Write(const ADest: TMemIniFile;
  const AFormatSettings: TFormatSettings);
var
vMm : TMeasureMode;
begin
...
procedure TmilDbExtention.AfterCreate;
begin
  inherited;
  fStartOptions := TIterOptions.Create;
  fInitRegisters := TMilandrRev8Registers.Create(nil);
  fFatalError := false;
end;

procedure TmilDbExtention.BeforeDestroy;
begin
...
class function TmilDbExtention.DefaultIterationClass: TIterationClass;
begin
  Result := TMilIteration<TSolution, TMeasure<TSolution>>;
end;

function TmilDbExtention.ReadPosData(const ASource: TMemIniFile;
...
function TmilDbExtention.UpdatePosData(const ADB: TEmbSQLDataBase;
...
function TmilDbExtention.UpdatePosData(AFields : TUpdatedFields): Boolean;
var
vDB : TEmbSQLDataBase;
begin
  Result := False;
...
procedure TIterOptions.ReadFromCardOptions(const ASource : TmilProcessDBOptions);
var
vMm : TMeasureMode;
begin
  ControlOptions.MeasureMode  := ASource.ControlOptions.MeasureMode;
...
class function TMilIterationBase.CreateRecord(const ADB: TEmbSQLDataBase; out oNum : Word): Boolean;
const
...
class function TMilIterationBase.CreateTables(const ADB: TEmbSQLDataBase): boolean;
begin
  try
    ADB.TransactionBegin;
...
class function TMilIterationBase.Delete(const ADB: TEmbSQLDataBase; ANum: Word): boolean;
const
...
class function TMilIterationBase.ReadCount(const ADB: TEmbSQLDataBase): Word;
const
...
class function TMilIterationBase.ReadState(const ADB: TEmbSQLDataBase; ANum : Word; out oState : TIterStates): boolean;
const
...
class function TMilIterationBase.WriteState(const ADB: TEmbSQLDataBase; ANum: Word;
...
class function TMilIterationBase.IORead(const ADB : TEmbSQLDataBase; ANum : Word; const Args : TIOArgs): boolean;
const
...
class function TMilIterationBase.IOWrite(const ADB : TEmbSQLDataBase; ANum : Word; const Args : TIOArgs): boolean;
const
c_SqlText = 'SELECT STATE FROM ITERATIONS WHERE NUM = :NUM;';
...
constructor TMilIteration<TCalc, TMeas>.Create(const ADBExt : TPositionDBExtention);
begin
...
destructor TMilIteration<TCalc, TMeas>.Destroy;
begin
...
function TMilIteration<TCalc, TMeas>.Read(const ADB: TEmbSQLDataBase; ANum: Word): boolean;
var
vFormatSettings: TFormatSettings;
begin
  vFormatSettings:= TFormatSettings.Create(1033);
...
function TMilIteration<TCalc, TMeas>.Read(const ADB: TEmbSQLDataBase;
                                          ANum: Word;
                                          const AFormatSettings: TFormatSettings): boolean;
var
vFO : TFieldOption;
...
function TMilIteration<TCalc, TMeas>.Write(const ADB: TEmbSQLDataBase; AIterState: Boolean;
...
procedure TMilIteration<TCalc, TMeas>.MoveCalcToMeas;
var
vIdx : Word;
...
constructor TIterPointsList.Create(const AOwner: TMilIterationBase);
begin
...
var
vIdx : Word;
vIter : TMilIteration<TSolution, TMeasure<TSolution>>;
begin
  vIdx := 0;
  Result := 0;
  vIter := TMilIteration<TSolution, TMeasure<TSolution>>(fIteration);
  while vIdx < Count do
  begin
...
constructor TMeasure<TCalc>.Create(const AIteration : TMilIterationBase);
begin
...
var
vSelfIdx : Byte;
vIter : TMilIteration<TCalc, TMeasure<TCalc>>;
begin
  vIter := TMilIteration<TCalc, TMeasure<TCalc>>(Iteration);
...
procedure TMilIterationRegisters.AfterCreate;
begin
end;

constructor TMilIterationRegisters.Create(const AIteration: TMilIterationBase);
begin
  inherited Create;
...
destructor TMilIterationRegisters.Destroy;
begin
  FreeAndNil(fRegisters);
  inherited;
end;

procedure TMilIterationRegisters.Read(const ASource: TMemIniFile; const ASection: string;
  const AFormatSettings: TFormatSettings);
begin
...
procedure TMilIterationRegisters.Write(const ADest: TMemIniFile; const ASection: string; const AFormatSettings: TFormatSettings);
begin
...
constructor TSolution.Create(const AIteration: TMilIterationBase; const ARootIdx : Byte);
begin
  inherited Create(AIteration);
  fRootIdx := ARootIdx;
end;
...
procedure TMilMeasPoint.MeasSuccInclude(AOption: TInRangeProcOption);
var
...
constructor TMeasures<T>.Create(const AIteration: TMilIterationBase);
begin
  inherited Create(true);
  fIteration := AIteration;
end;
