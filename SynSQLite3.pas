...

{
  ...
  ***** END LICENSE BLOCK *****
       ...
       SQLite3 3.7.14.1 database engine
      **********************************
...
{$I Synopse_17.inc} // define HASINLINE USETYPEINFO CPU32 CPU64 OWNNORMTOUPPER
...
type
  ...
  public
    ...
    procedure SaveBlobFieldToStream(Col: integer; AStream: TMemoryStream);
...
begin // called only by some obscure FTS3 functions (normal code use dedicated functions)
  result := SynCommons_17.StrLen(pointer(p));
end;
...
begin
  ...
  while n>0 do
  begin
    if not WriteFile(F.h,buf^,n,cardinal(result),nil) then
    begin
...
begin
  ...
  dec(buflen,result);
  if buflen>0 then
  begin // remaining bytes are set to 0
    inc(buf,result);
...
begin
  if argc=2 then
    with PConcatRec(sqlite3_aggregate_context(Context,sizeof(TConcatRec)))^ do begin
      ...
      txtlen := SynCommons_17.strlen(txt);
      if result=nil then
        GetMem(result,txtlen+1)
      else begin
        sep := sqlite3_value_text(argv[1]);
        seplen := SynCommons_17.strlen(sep);
...
begin // SQL function: RawUTF8DynArrayContainsCase/NoCase(BlobField,'Text') returning a boolean
  ...
  if Blob<>nil then begin
    ...
    if RawUTF8DynArrayLoadFromContains(Blob,Value,SynCommons_17.StrLen(Value),
...
procedure TSQLRequest.SaveBlobFieldToStream(Col: integer; AStream : TMemoryStream);
var p : PAnsiChar;
begin
  if cardinal(Col)>=cardinal(FieldCount) then
    raise ESQLite3Exception.Create(RequestDB, SQLITE_RANGE);
  p := sqlite3_column_blob(Request,Col);
  AStream.Write(p^, sqlite3_column_bytes(Request,Col));
end;
...
begin
  ...
  SetString(result,P,SynCommons_17.StrLen(P));
end;
